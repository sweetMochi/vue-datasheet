...
中止確認（`AbortConfirmDialog`）與重新解析的確認都已經做成對話框。「活的數字」那條規則有
`AbortConfirmDialog.test.ts` 守著
...
一句話：測試只證明前端不再派送事件，後端停止運算是依契約推論的，沒有對真實 uvicorn 驗過

### 六、資料送出給後端的規格不明確

mock 後端只有上傳、解析、健康檢查三支。真實系統一定有接收端，**只是這份題目沒有給規格**。
目前的做法是不對規格做假設：前端做完整驗證後把 `submitPayload` 輸出到 console，不打任何 API
...
payload 的形狀、分次送出、送出後能不能再改這幾件事，規格一到也都可能要改，
列在 log/08 的[要問出題方的](log/08-資料送出規格不明確.md#要問出題方的)
