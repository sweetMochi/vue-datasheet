...
原本 `resolveStatus` 把 `touched` 也當成處理完，實際用起來是個 bug，不只是疑慮：
`@input` 每個按鍵都會寫入，第一個字打下去狀態就變成 `ok`，
那一列在「需要你處理」裡連同輸入框一起被卸載 —— **使用者只打得進一個字**。
補缺漏欄位、「都不對，自己填」都會踩到
...
| ------------------------ | ------------------------------------------------------- |
| 改值                     | 不變，要按確認才變 `ok`                                 |
| 缺漏欄位補上值           | `unconfirmed`（「已補值 · 請確認」），確認後變 `ok`     |
| 確認或挑過候選之後又改值 | `setValue` 清掉 `confirmed`，回到原本的狀態，要重新確認 |
| 高把握度欄位改值         | 還是 `ok`，它們本來就不需要確認                         |

**為什麼另開 `unconfirmed`，不併進 `lowConfidence`**：只差一個判斷就能併，但畫面會說「系統把握度低」，
而實際情況是系統沒抽到、值是使用者填的。沿用黃色，只換標註與說明，
先前擔心的「第五種要用什麼顏色」就不成問題

**為什麼改值要清掉確認**：使用者確認的是舊的值。不清掉的話，`submitPayload` 會替一個沒人看過的值標上
`confirmed: true`，下游如果拿它當「有人核可過」，這筆資料就在說謊

還沒把握的是操作手感：在「全部」裡改一列確認過的欄位，第一個字打下去色條和確認鈕就會回來，
待處理數加 1。資料是對的，但可能讓人覺得「明明處理過了」。
如果實際使用常常改了又改回原值，可以改成記住確認時的值（`confirmedValue`），值相同就算已確認
...
mock 後端只有上傳、解析、健康檢查三支。真實系統一定有接收端，**只是這份題目沒有給規格**。
目前的做法是不對規格做假設：前端做完整驗證後把 `submitPayload` 顯示出來，不打任何 API
（取捨見 [log/08](log/08-資料送出規格不明確.md)）

那個決定本身我有把握——它不發明任何不存在的東西。沒把握的是**規格到手時，現在寫的有多少要改**

**最該注意的是一個看不見的不對稱**

上傳與解析這兩條路徑的錯誤處理是想過的，光錯誤碼就有五種：

```
DOCUMENT_EXPIRED / BAD_REQUEST / HTTP_ERROR / CONNECTION_LOST / STREAM_TRUNCATED
```

送出這條路徑**一個都沒有**。不是因為我判斷它不需要，是因為現在沒有任何東西會失敗。
這個空白今天完全看不出來——畫面跑得很順，測試也全綠——但端點一接上就立刻成為缺口：

- 送出失敗要顯示什麼？使用者剛審完 120 個欄位，不能只給一句「失敗」
- 失敗後 `phase` 該停在哪？`submitted` 還是退回 `review`？
- 重複送出怎麼辦？現在 `markSubmitted()` 沒有任何防重複，連送兩次不會有任何抱怨
- 送出中的狀態呢？`ReviewPhase` 沒有 `submitting`，上百欄位的請求不會是瞬間的

**payload 的形狀是為了給人看而設計的**

現在帶 `confirmed` 與 `edited`，因為這份 payload 唯一的讀者是螢幕前的人。真實後端可能只要
`id` 與 `value`，也可能反過來要更多（稽核軌跡、審核者、時間戳）。這個形狀撐不住任何審查

**還有兩件會牽動架構的事**

- **分次送出**：上百個欄位的文件，審核員很可能分兩天處理。若要支援，`document_id` 的生命週期
  就不能是後端 README 寫的「服務重啟即失效」，前端也需要草稿的持久化——那會推翻
  「不裝 `vue-router`、不做持久化」這個決定
- **送出後能不能再改**：現在 `backToReview()` 是無條件的，因為東西沒有真的送到哪裡。
  若真實流程送出即鎖定，這個出口要拿掉，「已送出」要變成真的終點
