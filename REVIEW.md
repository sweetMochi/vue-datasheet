...
`useVirtualList` 在元件規劃裡但沒實作，因為**還沒用 `?field_count=300` 實測過是否真的卡**。
沒測就先做，是拿一個確定的複雜度去換一個不確定的效能問題

真的要做的話，難點在於分組區段標題讓列高不一致，所以是可變高度的虛擬捲動，不是固定 `itemHeight`。
而且會犧牲瀏覽器原生的 Ctrl+F——對一個「找某個欄位」是核心動作的畫面來說，這個代價不小

備案：先做「只看需要處理的」預設篩選（已實作），把常態的可見列數壓到十幾列。
真正需要看 300 列的情境有多常見，也是想問出題方的一題
