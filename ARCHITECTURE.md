...
| 步驟   | 行為                                                                   |
| ------ | ---------------------------------------------------------------------- |
| 送出前 | `canSubmit` 為真才動作。送出鈕不灰掉，按下去才說明缺哪幾個             |
| 送出時 | `markSubmitted()` 只推進 `phase`，欄位與草稿都不動                     |
| 送出後 | 畫面顯示 `submitPayload`，並明講規格未定、未實際送出，不寫「送出成功」 |
| 反悔   | `backToReview()` 退回 `review`，其他 phase 呼叫無作用                  |

`submitted` 刻意不是死路：東西沒有真的送到任何地方，把使用者鎖在終點畫面是在演戲
