...
| 測試                                    | ✅ 126 支，涵蓋 SSE 分幀、HTTP 狀態分流、分組順序、送出阻擋、中止、中途失敗、重設、重新解析的資料保護、送出、樣式設定、一列的五種狀態、篩選與導覽、送出流程、中止確認與上傳 |
...
送出也接上了：確認對話框 → `console.table(payload)` → 跳窗提示已送出，資料留在畫面上
...
| 關閉   | 只關掉對話框，資料不清，`phase` 停在 `submitted`                           |

送出後欄位仍可編輯、可以再送一次。畫面上沒有「處理下一份」的出口，要換文件就重新整理頁面。
送出後能不能再改、怎麼回到某一份文件，都列在
[log/08 的「要問出題方的」](log/08-資料送出規格不明確.md#要問出題方的)
...
| 送出後資料留著，還能改、還能再送           | 有人在送出後清空資料，替還沒有規格的事做了決定           |
| `reset()` 回到 idle，清得乾淨              | 有人只清欄位忘了清 `document`，重新上傳會沿用舊的        |
