...
資料層（型別、傳輸、狀態）已完成並有測試覆蓋。審核畫面已經切版完成，長得跟 `log/design/01-解析中.png` 一致：三區骨架、群組導覽、「需要你處理／全部」分段、搜尋、sticky 區段標題，欄位的編輯／確認／挑候選／重設都能用。送出流程也完成了：確認對話框 → payload 輸出到 console → 跳窗提示已送出，資料留在畫面上。上傳、中止確認、解析中斷也都切好了 —— 四張設計稿全部落地
