...
### AI產生的介面最嚴重的三個問題，以及自己的版本怎麼處理

![AI產生的介面](subject/AI產生的介面_參考.png)

對於舊設計的反饋：

1. 分頁 UI 樣式對於逐項稽核是不友善的設計模式
2. tab 標籤：全部、必填、營養標示等，應屬於不同層級的過濾方式
3. 多重選取的框框，以當前的示意圖無法得知其作用為何
4. 下拉選單：群組、狀態等應屬於不同層級表示
5. 每筆資料應著重於 `編輯欄位的行為` 過多的 tag 和欄位會造成失焦

新版的介面著重於：

1. 關注使用者需確認的資料 `需要你處理`
2. `左方群組列` 可以快速切換關注的資料範圍
3. `上方功能列` 區分成「需要你處理」和「全部」，會根據當前篩選狀態更新數值
4. 逐筆修改只需向下捲動，減少額外換頁操作
5. 移除排序，使用者關注的是資料群組是否分類顯示

以下 AI 寫得很好，故僅修改部分文字描述

一列要承載標籤、值、把握度、確認狀態、必填、缺漏、候選、群組、頁碼九樣東西。平均分配等於都不重要，所以分三層：

| 資料列區域 | 範例                        | 呈現                  |
| ---------- | --------------------------- | --------------------- |
| 色帶顯示   | 這列需不需要我處理 (無色帶) | 左側 3px 色條，有三種 |
...
**直接看成品**（前端 build 後交給 nginx，跟評審看到的一致）：
...
- **資料送出給後端的規格不明確**。mock 後端只有上傳、解析、健康檢查三支，但需求明講必填未填「不能就這樣送出去」——真實系統一定有接收端，只是這份題目沒給規格。目前做法是不對規格做假設：前端完整驗證後把 payload 輸出到 console，並說明未實際送出（[log/08](log/08-資料送出規格不明確.md)）
