...
| 上傳 / 解析中 / 審核介面 | 🟡 進行中 | 欄位列已切版並接上編輯／確認／挑候選／重設；版面骨架、群組導覽、篩選、送出尚未（[元件配置](ARCHITECTURE.md#元件配置)）                                                        |
| 防護性測試               | ✅ 77 支  | SSE 分幀、HTTP 狀態分流、中止、解析中途失敗、必填缺漏擋送出、候選答案挑選、重設、重新解析的資料保護、送出、樣式設定、一列的四種狀態（[清單](ARCHITECTURE.md#測試防的是什麼)） |
...
資料層（型別、傳輸、狀態）已完成並有測試覆蓋。欄位清單已經換成切好版的 `FieldRow`，編輯、確認、挑候選、重設都能用。`App.vue` 裡上傳與進度那兩段還是原生 HTML、沒有版面，群組導覽與篩選也還沒接上 —— 目前是一條很長的清單
